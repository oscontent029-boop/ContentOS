...
{n:'09',title:'IA de Métricas',desc:'La IA analiza los números y te dice qué funciona, qué no, y qué oportunidades hay.',long:'La IA de Métricas interpreta los números del dashboard: fortalezas, oportunidades, alertas y recomendaciones. No trabaja sobre una impresión tuya, sino sobre los datos reales del período — lee la misma fuente que ves en el Dashboard, así que el análisis nunca contradice tus métricas.',image:'imagenes/ia-metricas.png',video:'videos/ia-metricas.mp4',color:'var(--pink)',bg:'rgba(214,135,183,.18)',icon:'<path d="M4 4h16v13.5H8.5L4 21z"/><path d="M8 9h8M8 12.5h5"/>'}, 
 ...
 {n:'11',title:'Estrategia',desc:'El plan general de la marca: objetivo, público, tono y pilares de contenido.',long:'La estrategia es la base de toda la marca: objetivo, público objetivo, tono de comunicación y pilares de contenido. No es un campo decorativo — es el contexto que usa el Centro de IA para generar todo, así que cuanto más completa esté, mejores resultados devuelve.',image:'imagenes/estrategia.png',video:'videos/estrategia.mp4',color:'var(--violet-light)',bg:'rgba(92,27,87,.25)',icon:'<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1"/>'},
...
const openModModal = (i) => {
  ...
  if(m.video && m.image){
    modModalMedia.innerHTML = `
      ...
      <video src="${m.video}" muted loop playsinline preload="metadata" class="mod-modal-video"></video>
      <div class="mod-modal-toggle">
        <button type="button" class="mod-modal-toggle-btn active" data-show="image">Foto</button>
        <button type="button" class="mod-modal-toggle-btn" data-show="video">Video</button>
      </div>`;
...
const closeModModal = () => {
  const v = modModalMedia.querySelector('video');
  if(v) v.pause();
